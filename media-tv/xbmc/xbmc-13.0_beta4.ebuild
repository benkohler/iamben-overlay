...
LICENSE="GPL-2"
SLOT="0"
IUSE="airplay alsa altivec avahi bluetooth bluray caps cec css debug +fishbmc gles goom joystick midi mysql neon nfs +opengl profile +projectm pulseaudio pvr +rsxs rtmp +samba +sdl sse sse2 sftp udev upnp +usb vaapi vdpau webserver +X +xrandr"
REQUIRED_USE="
	pvr? ( mysql )
	rsxs? ( X )
	X? ( sdl )
...
COMMON_DEPEND="${PYTHON_DEPS}
	...
	sdl? (
		media-libs/sdl-gfx
		>=media-libs/sdl-image-1.2.10:0[gif,jpeg,png]
...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-13.0_alpha9-nomythtv.patch
